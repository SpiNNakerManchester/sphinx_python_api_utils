...
def write_file(name, text, destdir, dryrun, force, suffix):
    """Write the output file for module/package <name>."""
    fname = path.join(destdir, '%s.%s' % (name, suffix))
    if dryrun:
        print 'Would create file %s.' % fname
        return
    if not force and path.isfile(fname):
...
def create_package_file(root, master_package, subroot, py_files,
    ...
    submods = [path.splitext(sub)[0] for sub in py_files
               if not shall_skip(path.join(root, sub), includeprivate)
               and sub != INITPY]
    if submods:
...
def create_modules_toc_file(modules, header, maxdepth, destdir, dryrun, force,
    ...
    write_file(name, text, destdir, dryrun, force, suffix)


def shall_skip(module, includeprivate):
